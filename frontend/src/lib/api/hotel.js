...
export async function getAmenities() {
  try {
    const res = await axios.get("/api/hotel/amenity", {
      withCredentials: true,
    });
    if (res.status === 200) {
...
export async function associateAmenities(hotel_id, array_amenity_id) {
  ...
  try {
    const res = await axios.post(
      `/api/hotel/amenity/associate/${hotel_id}`,
      body,
      config
    );
    if (res.status === 200) {
...
export async function checkAvailability(rooms, date_in, date_out, hotel_id) {
  ...
  try {
    const res = await axios.get(
      `/api/hotel/check?hotel_id=${hotel_id}&date_in=${date_in}&date_out=${date_out}`,
      config
    );
    if (res.status === 200) {
